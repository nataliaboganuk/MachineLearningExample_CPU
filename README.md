...
System Requirements table — Docker Desktop, Minikube, kubectl, k6, and an NGC account. (I worked on Ubuntu Desktop 24.04 Linux).

10 numbered steps in strict order:

1 Start Minikube with enough CPU/RAM
...
4 Check for all processes to be running

5 Upload the CSV File to the MinIO Bucket via the web console (WebUI)

6 Open JupyterLab in a browser

7 Upload and run the notebook (not locally on the device)— with a table explaining what each section does and what a successful upload looks like

8 Open the **`triton_mathmodel_k6_test.js`** in VSCode and align **`TRITON_HOST`** and **`TRITON_PORT`** it must be the same as shows in the terminal.

9 Apply **`third_deployment.yaml`** — deploys Triton, which auto-loads the model from MinIO

10 Verify Triton health and fire a test inference with curl

11 Run the k6 load test with the correct environment variables
...
> **Hardware:** The Triton deployment uses the image (`tritonserver:26.02-py3`).
...
│   ├── third_deployment.yaml    # Stage 3 — Triton Inference Server
└── K6_Test_Scripts/
...
Open a terminal and start a local Kubernetes cluster with enough resources:

```bash
minikube start --cpus=4 --memory=10240 --disk-size=30g --driver=docker --container-runtime=docker
```
...
minikube service minio-api -n mlops --url
```

---

### Step 7 — Deploy Stage 3: Triton Inference Server

Now that the model artefacts are in MinIO, deploy Triton:

```bash
kubectl apply -f K8s_Manifests/third_deployment.yaml
```

Triton will:
1. Pull the CPU-only image `nvcr.io/nvidia/tritonserver:26.02-py3` (this is large — ~8 GB — and may take several minutes to pull (download) on first run).
...
### Step 8 — Verify Triton Is Serving Predictions
...
- Confirm the model files exist in MinIO at the correct paths (Step 7 checklist above).
...
- The notebook uses the cluster-internal DNS name `minio-api.mlops.svc.cluster.local:9000`. This only works when the notebook is running *inside* the cluster (i.e. in JupyterLab deployed in Step 3). Do not run the notebook locally on your laptop.
